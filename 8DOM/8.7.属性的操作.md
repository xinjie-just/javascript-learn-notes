...
```javascript
// HTML 代码：<body bgcolor="yellow" onload="">
document.body.attributes[0];
document.body.attributes.bgcolor;
document.body.attributes['ONLOAD'];
```

**上例的三种方法，返回的都是属性节点对象，而不是属性值。**

属性节点对象有 `name` 和 `value` 属性，对应该属性的属性名和属性值，等同于 `nodeName` 属性和 `nodeValue` 属性。

```javascript
// HTML 代码：<div id="mydiv">
...
上例中，`a` 元素标签的属性 `id` 和 `href`，自动成为节点对象的属性。
...
上例中，`setAttribute` 方法直接使用 `class` 作为属性名，不用写成 `className`。
...
```javascript
// HTML 代码：<div id="div1" align="left">
...
```javascript
// HTML 代码：<button>Hello World</button>
let b = document.querySelector('button');
b.setAttribute('name', 'myButton');
b.setAttribute('disabled', true);
```

上例中，`button` 元素的 `name` 属性被设成 `myButton`，`disabled` 属性被设成 `true`。
...
上例检查 div 节点是否含有 align 属性。如果有，则设置为居中对齐。
...
```javascript
// HTML 代码：<div id="div1" align="left" width="200px">
document.getElementById('div1').removeAttribute('align');
// 现在的 HTML 代码：<div id="div1" width="200px">
```
...
上例为 `<div>` 元素自定义了 `foo` 属性，然后可以用 `getAttribute()` 和 `setAttribute()` 读写这个属性。
...
上例中，通过 `dataset.foo` 读写 `data-foo` 属性。
