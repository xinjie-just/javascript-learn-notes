...
> - `writable` 是否可改变（是否可写），默认是 `true`。
> - `enumerable` 是否可遍历，默认是 `true`。如果设为 `false`，会使得某些操作（比如 `for...in` 循环、`Object.keys()`）跳过该属性。
> - `configurable` 是否可配置，默认是 `true`，如果设为 `false`，将阻止某些操作改写该属性，比如无法删除该属性，也不得改变该属性的属性描述对象（`value` 属性除外）。也就是说，`configurable` 属性控制了属性描述对象的可写性。
